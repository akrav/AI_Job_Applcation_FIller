...
describeMaybe('POST /api/v1/profile/onboard (integration)', () => {
  beforeAll(async () => {
    ({ default: app } = await import('../index.js'));
  });

  it('saves initial profile data for authenticated user', async () => {
    const admin = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
    const unique = Date.now();
    const email = `onboard-${unique}@gmail.com`;
    const password = 'Password123!';
    const name = 'Onboard User';
    ...
    const initialProfile = { work_experience: '5 years', skills: ['js', 'react'] };
    ...
    const { data: mb } = await admin
      ...
      .maybeSingle();
    expect(mb?.data?.work_experience).toBe('5 years');
